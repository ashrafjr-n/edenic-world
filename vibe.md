...
- Rely on the running dev server (Next.js/Turbopack HMR) for changes to appear. If the dev server shows an error after your change, say so in one line in the report.
- Do not install, remove, or upgrade a package unless the current command actually needs it. If a step could arguably use a new library, ask or flag it instead of adding it silently.
...
- `CLAUDE.md` and `vibe.md` must always stay tracked and pushed — never add them to `.gitignore` and never `git rm`/untrack them.
- Never edit `.gitignore` or untrack/remove files from git for reasons unrelated to the current command without flagging it first.
